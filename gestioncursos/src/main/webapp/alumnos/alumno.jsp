	...
	%>
		...
		%>
			<form name="" id="" method='post' class="form-horizontal"
				...
				<div class="form-group">
					<label class="col-xs-2" >Género:</label>
					<div class="col-xs-10">
						<input type="radio" 
							name="<%=Constantes.PAR_GENERO%>" class="form-control"
							id="<%=Constantes.PAR_GENERO%>" <%=Genero.MASCULINO == alumno.getGenero() ? "checked" : "" %>
							value="<%=Genero.MASCULINO.getCodigo() %>"
							/><%=Genero.MASCULINO.getValor() %>
							<input type="radio" 
							name="<%=Constantes.PAR_GENERO%>" class="form-control"
							id="<%=Constantes.PAR_GENERO%>" <%=Genero.FEMENINO == alumno.getGenero() ? "checked" : "" %>
							value="<%=Genero.FEMENINO.getCodigo() %>"
							/><%=Genero.FEMENINO.getValor() %>
							<input type="radio" 
							name="<%=Constantes.PAR_GENERO%>" class="form-control"
							id="<%=Constantes.PAR_GENERO%>" <%=Genero.OTROS == alumno.getGenero() ? "checked" : "" %> 
				...
				<div class="form-group">
					...
					<div class="col-xs-10">
						<input type="checkbox" 
							name="<%=Constantes.PAR_IDIOMA%>" class="form-control"
							id="<%=Constantes.PAR_IDIOMA%>" <%= alumno.getIdiomas().contains(Idioma.CASTELLANO)  ? "checked" : "" %>
							value="<%=Idioma.CASTELLANO.getCodigo() %>"
							/><%=Idioma.CASTELLANO.getNombre() %>
						<input type="checkbox" 
							name="<%=Constantes.PAR_IDIOMA%>" class="form-control"
							id="<%=Constantes.PAR_IDIOMA%>" <%= alumno.getIdiomas().contains(Idioma.INGLES) ? "checked" : "" %>
							value="<%=Idioma.INGLES.getCodigo() %>"
							/><%=Idioma.INGLES.getNombre() %>
							<input type="checkbox" 
							name="<%=Constantes.PAR_IDIOMA%>" class="form-control"
							id="<%=Constantes.PAR_IDIOMA%>" <%= alumno.getIdiomas().contains(Idioma.EUSKERA) ? "checked" : "" %>  
